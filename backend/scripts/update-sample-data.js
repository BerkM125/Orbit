...
async function updateUserProfiles() {
	try {
		...
		for (let i = 0; i < profiles.length; i++) {
			...
			if (updateError) {
				console.error(
					`❌ Error updating ${profile.first_name} ${profile.last_name}:`,
					updateError
				);
			} else {
				console.log(
					`✅ Updated ${profile.first_name} ${profile.last_name} - ${company} at (${location.lat}, ${location.lng})`
				);
			}
		}
		...
		if (!verifyError && updatedProfiles) {
			...
			updatedProfiles.forEach((profile) => {
				console.log(
					`${profile.first_name} ${profile.last_name} - ${profile.company} at (${profile.latitude}, ${profile.longitude})`
				);
			});
		}
	} catch (error) {
